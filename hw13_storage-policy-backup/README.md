...
3.  **Создание бакетов:** После успешного запуска MinIO, `null_resource` "minio_buckets" вызывает Python-скрипт `base-infra/scripts/minio_bucket_create.py`.
4.  **Скрипт `minio_bucket_create.py`:**
    *   Подключается к каждому инстансу MinIO.
    *   Проверяет наличие необходимых бакетов (`clickhouse-backups` и `clickhouse-storage-bucket`).
    *   Если бакет не существует, он его создает. Если существует — ничего не делает.
5.  **Запуск остальных ресурсов:** Только после успешного выполнения скрипта и создания бакетов Terraform продолжает развертывание остальных ресурсов, включая ClickHouse.
...
Для работы скрипта необходимо установить библиотеку `minio`:
```bash
