...
# Local MinIO for backup on Samsung T7 SSD (local_ssd_backup mode)
...
resource "docker_container" "minio_local_backup" {
  ...
  ports {
    internal = 9001
    external = var.local_minio_port + 11
  }
  mounts {
    source = abspath(var.local_minio_path)
    target = "/data"
...
resource "null_resource" "wait_for_local_backup_minio" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      for i in {1..30}; do
        if curl -s "http://localhost:${var.local_minio_port + 10}/minio/health/live" &> /dev/null; then
...
# Remote MinIO for Backups (enabled via variable)
...
# Create backup bucket in same local MinIO (S3 storage + backup in same MinIO)
resource "null_resource" "local_s3_backup_bucket" {
  count = var.storage_type == "s3_ssd" && !var.enable_remote_backup ? 1 : 0
  provisioner "local-exec" {
    command = "mc alias set local_storage http://localhost:${var.local_minio_port} ${var.minio_root_user} ${var.minio_root_password} --api S3v4 && mc mb --ignore-existing local_storage/${var.bucket_backup}"
  }
  depends_on = [null_resource.wait_for_local_minio]
}
...
resource "null_resource" "local_backup_minio_bucket" {
  count = var.storage_type == "local_ssd_backup" ? 1 : 0
  provisioner "local-exec" {
    command = "mc alias set local_backup http://localhost:${var.local_minio_port + 10} ${var.minio_root_user} ${var.minio_root_password} --api S3v4 && mc mb --ignore-existing local_backup/${var.bucket_backup}"
  }
  depends_on = [null_resource.wait_for_local_backup_minio]
}
...
resource "docker_container" "clickhouse_backup_remote" {
  ...
  depends_on = [docker_container.ch_nodes, null_resource.remote_minio_bucket]
}
...
resource "docker_container" "clickhouse_backup_local" {
  ...
  env = [
    ...
    "S3_ACCESS_KEY=${var.minio_root_user}",
    "S3_SECRET_KEY=${var.minio_root_password}",
    "S3_ENDPOINT=http://host.docker.internal:${var.local_minio_port + 10}",
    "S3_REGION=us-east-1",
    "S3_DISABLE_SSL=true",
    "S3_FORCE_PATH_STYLE=true"
  ]
  depends_on = [docker_container.ch_nodes, null_resource.local_backup_minio_bucket]
}
...
resource "docker_container" "clickhouse_backup_s3" {
  ...
  env = [
    ...
    "S3_ACCESS_KEY=${var.minio_root_user}",
    "S3_SECRET_KEY=${var.minio_root_password}",
    "S3_ENDPOINT=http://host.docker.internal:${var.local_minio_port}",
    "S3_REGION=us-east-1",
    "S3_DISABLE_SSL=true",
    "S3_FORCE_PATH_STYLE=true"
  ]
  depends_on = [docker_container.ch_nodes, null_resource.local_storage_minio_bucket]
}